...
class Dataprocess(Dataset):
    def __init__(self, args, state='train', index=0):
        ...
        if not os.path.exists("./word2vec/word2vec_embedding_"+self.dataset+".npy") and ("./word2vec/word_ids_list_"+self.dataset+".npz"): 
            ...
            features = self.get_features(args, texts, labels, word_ids_list)
        else:
    ...
    ########################################################################
    def get_features(self, args, texts, labels, word_ids_list):
        features = []        
        ...
        for id, sent in enumerate(texts):
            ...
            feature = {
                        'texts': text, 'input_ids': input_ids, 'attention_masks': attention_masks, 

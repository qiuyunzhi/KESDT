...
def evaluate(args, model, data_loader):
    ...
    with torch.no_grad():  # 计算的结构在计算图中,可以进行梯度反转等操作
        for data in data_loader:
            input_ids, attention_masks, labels = tuple(t.to(args.device) for t in data)  # 将三个tuple元素传到服务器中            
...
def train(args, model, train_loader, dev_loader, optimizer):
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in train_loader:
            input_ids, attention_masks, labels = tuple(t.to(args.device) for t in batch)  # 将三个tuple元素传到服务器中
             
            # 1、前向传播
            y_pred = model(input_ids, attention_masks)              
            loss = criterion(y_pred, labels)   #输入的y_pred=[batch数，类别数]=[16,2], label=[类别数]=[16]
...
def dataLoader(args, state, index):
    ...
    for i, dict_line in enumerate(list_all):
        input_id, attention_mask, label = dict_line.get('input_ids'), dict_line.get('attention_masks'), dict_line.get("label_ids")    # label 是一个包含1861个int的list
    ...
    datas = TensorDataset(input_ids, attention_masks, labels)

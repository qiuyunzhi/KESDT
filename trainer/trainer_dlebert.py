...
def evaluate(args, model, data_loader):
    ...
    with torch.no_grad():  # 计算的结构在计算图中,可以进行梯度反转等操作
        for data in data_loader:
            input_ids, attention_masks, token_type_ids, labels, word_ids, word_masks = tuple(t.to(args.device) for t in data)  # 将三个tuple元素传到服务器中            
            y_pred = model(input_ids, attention_masks, token_type_ids, word_ids, word_masks) 
...
def train(args, model, train_loader, dev_loader, optimizer):
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in train_loader:
            input_ids, attention_masks, token_type_ids, labels, word_ids, word_masks = tuple(t.to(args.device) for t in batch)  # 将三个tuple元素传到服务器中
...
def dataLoader(args, state, index):
    ...
    for i, dict_line in enumerate(list_all):
        input_id, attention_mask, token_type_id, word_id, word_mask, label = dict_line.get('input_ids'), dict_line.get('attention_masks'), dict_line.get("token_type_ids"), dict_line.get("word_ids"), dict_line.get("word_masks"), dict_line.get("label_ids")    # label 是一个包含1861个int的list
    ...
    word_masks = (word_ids != 0)
    #word_masks = torch.LongTensor(word_masks)   # 之前算的是int类型，所以不用，需要用bool类型

    datas = TensorDataset(input_ids, attention_masks, token_type_ids, labels, word_ids, word_masks)
